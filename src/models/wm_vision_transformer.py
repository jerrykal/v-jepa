...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.pos_embed = nn.Parameter(
            torch.zeros(1, self.num_patches + grid_depth, embed_dim),
            requires_grad=False) #self.num_patches + grid_depth(action)
    ...
    def forward(self, x, encode_action, masks=None):
        """
        ...
        T_new = T // self.tubelet_size
        encode_action = encode_action[:, :T_new * self.tubelet_size, :]  # [B, T', tubelet, D]
        encode_action = encode_action.view(B, T_new, self.tubelet_size, D).mean(dim=2)  # [B, T', D]
        encode_action = encode_action.unsqueeze(-1)
        action_pos_embed = torch.zeros(1, T_new, D, device=x.device)

        # Tokenize input
        pos_embed = self.pos_embed
        if pos_embed is not None:
            pos_embed = self.interpolate_pos_encoding(x, pos_embed)
        x = self.patch_embed(x, flatten=3)
        # x => [B, T', D, P]
        # action => [B, T', D, 1]
        # Add encode_action as extra token
        x = torch.cat([x, encode_action], dim=-1)  #[B, T', D, P+1]
        x = x.permute(0, 1, 3, 2) #[B, T', P+1, D]
        x = x.reshape(x.size(0), -1, x.size(-1)) #[B, (T' * P+1), D]
        if pos_embed is not None:
            x += pos_embed
        B, N, D = x.shape
