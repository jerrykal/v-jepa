...
from evals.visualization_frozen.utils import(
    init_models,
...
def main(args_eval, resume_preempt=False):
    ...
    jepa_pth = cfgs_model.get("jepa_pth")
    ...
    # -- init model
    encoder, target_encoder, predictor, decoder = init_models(
        device=device,
    ...
    encoder, target_encoder, predictor, decoder \
    = load_pretrained_model(
        model_path={
            'jepa_pth':jepa_pth,
    ...
    with torch.amp.autocast("cuda", dtype=dtype, enabled=mixed_precision):
        ...
        def forward_context(    
            ...
            with torch.no_grad():
                ...
                if autoregressive:
                    ...
                    for _ in range(_t-1):
                        out_list = predictor([z_ctx], h, masks_e, masks_p, [None])
                        pred_tok = out_list[0]
        ...
        target_feature, _   = forward_target(clips)
        full_z              = forward_context(clips, target_feature, autoregressive=enable_autoregressive)
