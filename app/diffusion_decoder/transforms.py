...
def denormalize_clips(
    clips: torch.Tensor,
    mean: list[float] | torch.Tensor = [0.485, 0.456, 0.406],
    std: list[float] | torch.Tensor = [0.229, 0.224, 0.225],
):
    """Denormalize clips to range [0, 1]

    Args:
        clips (torch.Tensor): A tensor of shape (B, C, F, H, W).
        mean (list[float] | torch.Tensor, optional): The mean value to add. Defaults to [0.485, 0.456, 0.406].
        std (list[float] | torch.Tensor, optional): The std to multiply. Defaults to [0.229, 0.224, 0.225].

    Returns:
        torch.Tensor: Denormalized clips of shape (B, C, F, H, W).
    """
    ...
    clips = clips.clone().contiguous()
    mean = mean.to(clips.device)
    std = std.to(clips.device)

    B, C, F, H, W = clips.shape
    clips = clips.view(B, C, -1).permute(0, 2, 1)
    clips.mul_(std).add_(mean)
    clips = clips.permute(0, 2, 1).view(B, C, F, H, W)
    return clips
